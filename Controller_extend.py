...
class Controller_extend(Controller):
    ...
    def s(self, k, vehicle):
        ...
        velocity = max(vehicle.states[kk].velocity, self.epsilon)  # Prevent zero velocity
        k_i = vehicle.controller.states[kk].omega / velocity

        if k_i == 0:
            s_magnitude = 0
        else:
            sqrt_val = max(1 + (k_i ** 2) * (self.r + self.h * velocity) ** 2, 0)  # Ensure non-negative for sqrt
            s_magnitude = (-1 + math.sqrt(sqrt_val)) / max(k_i, self.epsilon)  # Prevent division by zero
    ...
    def updateState(self, k, T, prec, vehicle, a, w):
        ...
        if vehicle.first:
            # Inizializzazione degli errori e degli stati se è il primo veicolo
            ...
            # Calcolo di Alpha e altri parametri
            self.Alpha(k, prec, vehicle)

            # Aggiornamento degli errori con gestione dei NaN
            ...
            # Definizione di matrici e parametri
            kk = k - 1
            H_i_1 = np.array([
            ...
            delta_i = vehicle.states[kk].velocity * self.h * (
                        math.cos(vehicle.controller.states[kk].alpha) ** 2) * k_i_1
            ...
            # Calcolo di der_k con gestione delle divisioni per zero
            ...
            B_2 = np.clip(B_2_factor * vehicle.states[kk].velocity * (cos_alpha ** 2) * der_k, -1e10, 1e10)


            # Calcolo finale evitando NaN
            ...
            # Aggiornamento degli errori sulla velocità
    ...
    def B_1(self, prec, vehicle, k, T):
        ...
        velocity = max(prec.states[k].velocity, self.epsilon)  # Prevent zero velocity
        k_i = prec.controller.states[k].omega / velocity

        delta_k = vehicle.states[k].velocity * self.h * (math.cos(vehicle.controller.states[k].alpha) ** 2) * k_i
        der_k = ((prec.controller.states[k].omega / velocity)
                 - (prec.controller.states[k - 1].omega / max(prec.states[k - 1].velocity, self.epsilon))) / T
        B_1 = delta_k * der_k * np.dot(R, O)
        return B_1
    ...
    def get_acceleration_omega(self, prec, vehicle, k, T):
        ...
        # Creazione di Z_12 usando k1 e k2, come fatto prima
        Z_12 = np.array([
        ...
        cos_alpha = max(math.cos(vehicle.controller.states[k].alpha),
                        self.epsilon)  # Prevenzione di cos(alpha) troppo piccolo
        ...
        # Calcolo della matrice T_12_inv
        T_12_inv = self.T_12_inv(prec, vehicle, k)

        # Calcolo di B_1 usando la funzione già definita
        B_1 = self.B_1(prec, vehicle, k, T)

        # Somma dei termini per ottenere il vettore risultato
        Res = Z_12 + Z_34 + B_1

        # Calcolo della matrice A moltiplicando T_12_inv per il risultato
        A = np.dot(T_12_inv, Res)

        # Imposta accelerazione e omega nello stato successivo (kk)
